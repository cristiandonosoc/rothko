...
bool InitImguiRenderer(ImguiRenderer* imgui_renderer, Renderer* renderer, ImGuiIO* io) {
  ...
  LOG(DEBUG, "VIIIIIEW: %s", ToString(imgui_renderer->ubo.view).c_str());
...
PerFrameVector<RenderCommand> ImguiGetRenderCommands(ImguiRenderer* imgui_renderer) {
  ...
  for (int i = 0; i < draw_data->CmdListsCount; i++) {
    ...
    for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
      ...
      /* /1* IndexRange range = 0; *1/ */
      /* /1* range = PushOffset(range, base_index_offset + index_offset); *1/ */
      /* /1* range = PushSize(range, draw_cmd->ElemCount); *1/ */
      /* /1* total_size += GetSize(range); *1/ */
      /* /1* render_action.index_range = range; *1/ */
